...
- make require delegate to the environment
  - allows the env to intervene, update lists, etc.
- library data augments the current document
  - copy on write
  - initializes the data in the doc
  - adds a "imported" parameter to tie the data to the imported doc
  - if the imported doc is removed, it removes the associated data
    - scan data after load and prune obsolete imported data
...
** TODO value slider final value isn't always used
