...
- collaboration
  - each document gets local storage for user-local and user-private data
    - ":storage local" blocks are in the document, initialized for everyone, but stored locally
    - ":storage private" blocks are not in the document but are overlayed for the current user
    - createRef(item, parent) should make a reference and verify it's not public->private
