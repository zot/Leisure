* use promises for lazy code in Leisure so we can use Chrome's async support
* TODO don't use CSS inserted content for character sheets because it doesn't copy?
* TODO View on a block's content
** Edit a src block's content
** Edit a headline's content
** creates a local editor, like search does
* TODO Restructure code creation
