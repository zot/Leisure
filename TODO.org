...
- Collaboration engine
  - use meteor for collaboration
    - X put org data structure into mongo
    - X Use mongo instead of textContent to switch modes
    - X Use change processing to handle local structural changes
    - Handle node addition/removal
    - make data live in src blocks with language yaml or json
  - X detect where changes are, to avoid rerendering/recomputing data
...
- collaboration service
  - use meteor for collaboration
    ...
    - make data live in src blocks with language yaml or json
    - X nested chunks: {"_id": id, "topline": text, "children": [...]}
      - X a child can be a newline-terminated string or a chunk
  - detect changes to avoid unnecessary computation
    - only recompute data which has changed
    - only rerender elements which have changed
  - show users' cursors/selections
  - persistence: git
...
- data
  ...
  - internal representation
    - manipulated by monads
    - stored by ID in a hamt
    ...
    - JSON properties are converted to Leisure values
      - arrays become lists (until good vector support)
      - objects become hamts
    - query
      - use datalog, like datomic?
    - listener fires when data changes
- HTML/image data binding
  - make image dragging and collaboration use data binding -- maybe backbone
  - check observe.js, backbone, knockout, angularjs, ...
- security
