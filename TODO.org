* strict mode
** change primitives not to use arcuments.callee
...
* hide leading non-toplevel content in fancy mode
* add resourceBase=URL parameter for resource fallback
** helps  collaboration on standard resources
* collaboration clients should fetch unknown resources from master
* TODO Views that contain select Leisure content, like search does
* TODO Allow block results as input (key Babel feature)
  - Allows piping
...
* TODO Need fancy gizmos to control header args
