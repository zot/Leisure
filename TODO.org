* TODO [#A] move OT server into browser with WebRTC
* TODO [#A] toolbar buttons should not steal focus
* TODO [#A] backspace at the start of the line after a code block should remove only one newline
* TODO [#A] error rendering views -- both views in elisp readme are the same
* TODO [#A] links don't render an error if the type changes to a bad one
* TODO [#A] add trace call for let assigns in genLets
* TODO [#A] move debugging stuff into ast.coffee
* TODO [#A] use generated function name to indicate info in order to eliminate the L$info assignment
** provide a unique tag to CodeGenerator to append to generated function IDs
* TODO [#A] seems to be a problem with defCase
** present.type doesn't seem to override present.hasType
* TODO [#A] Make hash arguments that can position the document, etc.
* TODO [#A] Access viewdef for a given view
* TODO [#A] Viewdef and controler should render links to each other
* TODO [#A] Add finger trees to Leisure
* TODO [#A] generate debug and non-debug versions of each function
* TODO [#A] Vele character sheet computer
* TODO [#A] convert rest of defCases to defTypeCase (bind is problematic)
* TODO [#A] Leisure impl of defCase, defTypeCase, etc.
* TODO [#A] Need a way to access sourcemaps
** wrap generated code in a context function that initializes with the sourcemap
** make $F initialization put the context into the function
* BUG [#A] defWrapper generates faulty partial application code
* TODO [#A] Working source in a :working: drawer, c-s transfers it to src block
* TODO [#A] fix Leisure's sourcemap generation
* TODO [#A] codegen optimization
** initialize all properties in the constructor and in the same order
** remove try/catch -- delegate try contents to a nested function
* TODO [#A] make toJson cases for immutable.js data types
* TODO [#A] Macros that use IO don't recursively replace their contents
#+BEGIN_SRC leisure :results dynamic
defMacro 'ignoreFirst' \list . do
  print['first arg: ' (head list)]
  tail list

#defMacro 'ignoreFirst' \list . log s['first arg: ' (head list)]
#  tail list

simplify 'ignoreFirst blah head [1 2 3]' 
#+END_SRC
#+RESULTS:
: "first arg: token blah EMPTYFILE.lsr 1 12"
: 
: ["head" ["[" ["1" "2" "3"] "]"]]
...
* Done
  ...
  :END:
** DONE [#A] Make defTypeCase func.type that adds a method to the type's class
   CLOSED: [2017-05-16 Tue 01:32]
...
** DONE remove foldVar from do macro funcs
   CLOSED: [2017-05-14 Sun 10:32]
*** bundle args with cons instead of triple
