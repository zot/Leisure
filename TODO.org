* change :control blocks to use :var instead of @initializeView
* Use Object.create() instead of __proto__
* P2P should transmit document name to slaves if it's a public URL
* TODO Undo: for OT, register with UndoManager, for local, use replacements
* Finish the big arity substitution for primitives and codegen
* Write program to test new vele mechanics
* implement dofold
dofold i <- 0
  x <- range 0 100
  i + x
* TODO Martini button in Leisure bar that toggles the entire page
* TODO structural keybord commands
* TODO Make a drawer for "uncommitted" content
...
* TODO code blocks for handlebar partials
