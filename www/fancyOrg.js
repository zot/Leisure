...
(function() {
  var BS, DEL, DOCUMENT_POSITION_CONTAINED_BY, Drawer, END, ENTER, HL_TAGS, HOME, HTML, Headline, KW_BOILERPLATE, KW_INFO, KW_NAME, Keyword, Link, ListItem, Meat, PAGEDOWN, PAGEUP, Results, SimpleMarkup, Source, TAB, addComment, addKeyPress, allowEvents, atTextEnd, atTextStart, baseEnv, bindContent, boundarySpan, bsWillDestroyParent, changeResultType, checkCollapsed, checkEnterReparse, checkExtraNewline, checkSourceMod, checkStart, checkTestResults, childIndex, clearResults, codeBlockForNode, colonify, commentBlock, commentButton, commentHtml, content, contentSpan, createComment, createNotes, createResults, createTestCase, crossesHidden, curPos, currentLine, currentTextPosition, defaultBindings, defaultEnv, defaultMarkup, define, displaySource, documentTop, drawerRE, eatListItem, editedNode, emptyPresenter, escapeAttr, escapeHtml, escapePresentationHtml, executeCode, executeDef, executeSource, executeText, fancyCheckSourceMod, fancyExecuteDef, fancyOrg, findDomPosition, findKeyBinding, firstSlide, followingSpan, getCodeContainer, getDocumentOffset, getNodeSource, getOrgParent, getOrgType, getResultsForSource, getTextPosition, getType, handleKey, handleMutation, headlineRE, hideSlider, hideSlides, hlStars, hlineFor, htmlForResults, imagePath, invalidateOrgText, isCollapsed, isDef, isMonad, isOrContains, keyFuncs, keywordRE, lastOrgOffset, lastSlide, lazy, linePat, lz, makeSyncMonad, markupGuts, markupHeadline, markupHtml, markupLink, markupListContents, markupListItem, markupNewNode, markupNode, markupOrg, markupOrgWithNode, markupProperties, markupSimple, markupSource, matchLine, matchLineAt, modifiers, modifyingKey, nativeRange, needsNewline, newCodeContent, newCommentBox, nextOrgId, nextSlide, nonl, noteAttrs, oldRestorePosition, optionalBoundary, orgAttrs, orgEnv, orgNotebook, orgSrcAttrs, parseOrgMode, parseTags, presentValue, presenter, prevSlide, processResults, propsFor, psgn, recreateAstButtons, redrawAllIssues, redrawIssue, replaceCodeBlock, replacePresenter, replaceRelatedPresenter, replaceUnrelatedPresenter, reprocessResults, resolve, restorePosition, restoreSlide, resultsRE, resultsType, root, runAutotests, runMonad, runTest, rz, selectPrevious, setCurKeyBinding, setCurrentSlide, setMinMax, setShadowHtml, setSlideAt, setTheme, shouldCancelBS, shouldCancelDEL, show, showAst, showSliderButton, showSlides, slideBindings, slideMode, slideOffset, slideParent, splitLines, srcStartRE, swapMarkup, testResult, textNodeAfter, textNodeBefore, theme, toTestCaseButton, toggleComment, toggleDynamic, toggleSlides, topNode, unescapePresentationHtml, unwrap, updateChannels, watchNodeText, _, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6;
  ...
  noteAttrs = function(org) {
    var _ref7;
    if ((_ref7 = org.properties) != null ? _ref7.notes : void 0) {
      return "data-org-notes='" + org.properties.notes + "'";
    } else {
      return '';
    }
  };
  ...
  createNotes = function(node) {
    var noteSpec, _i, _len, _ref7, _results;
    watchNodeText(node, editedNode(node.id, node.id));
    _ref7 = node.getAttribute('data-org-notes').split(',');
    _results = [];
    for (_i = 0, _len = _ref7.length; _i < _len; _i++) {
      noteSpec = _ref7[_i];
      _results.push(console.log("NOTE FOR " + node.id + ": " + noteSpec));
    }
    return _results;
  };

  editedNode = function(mainId, editedId) {
    return function() {
      return console.log("EDITED NODE: " + mainId + " from " + editedId);
    };
  };
