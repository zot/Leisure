...
(function() {
  ...
  root.restorePosition = restorePosition = function(parent, block) {
    var c, container, end, endContainer, endOffset, newSlide, offset, r, sel, slide, slideIndex, sta, start, top, topPos, _ref6, _ref7;
    sel = getSelection();
    ...
    if ((sel != null ? sel.rangeCount : void 0) && slideIndex > -1) {
      ...
      _ref6 = findDomPosition(top, start), container = _ref6[0], sta = _ref6[1];
      ...
      if (start > -1 && (r = nativeRange(findDomPosition(top, start)))) {
        if (isCollapsed(r.startContainer)) {
          ...
          _ref7 = findDomPosition(top, end), endContainer = _ref7[0], endOffset = _ref7[1];
          if (endOffset === 0) {
  ...
  childIndex = function(parent, child) {
    var i, _i, _ref6;
    for (i = _i = 0, _ref6 = parent.children.length; 0 <= _ref6 ? _i < _ref6 : _i > _ref6; i = 0 <= _ref6 ? ++_i : --_i) {
  ...
  markupSource = function(org, name, intertext) {
    var codeBlock, contHtml, expected, lead, nameM, node, resText, result, srcContent, startHtml, testCase, trail, wrapper;
    ...
    wrapper += "</td></tr></table>";
    if (testCase = resultsType(org) === 'test' && expected && (resText != null)) {
      wrapper += '\n';
    }
    ...
    if (testCase) {
      return startHtml + ("onclick='Leisure.toggleTestCase(event)' org-test='" + (testResult(expected.content(), resText)) + "' title='<b>Expected:</b> " + (escapeAttr(expected.content())) + "' expected='" + (escapeAttr(expected.content())) + "' " + result);
    } else {
      return '<div>' + startHtml + result + '\n</div>';
    }
  };
  ...
  root.toggleTestCase = function(evt) {
    ...
    if (node) {
      return replaceCodeBlock(node, changeResultType(node.textContent, 'dynamic'));
    }
  };

  replaceCodeBlock = function(node, text) {
    return restorePosition(null, function() {
      var n, newNode, _i, _j, _len, _len1, _ref6, _ref7,
        _this = this;
      newNode = $(markupNewNode(parseOrgMode(text).children[0]))[0];
      $(node).replaceWith(newNode);
      _ref6 = $(newNode).find('[data-org-src]');
      for (_i = 0, _len = _ref6.length; _i < _len; _i++) {
        n = _ref6[_i];
        recreateAstButtons(parent, n);
      }
      _ref7 = $(newNode).find('.resultscontent');
      ...
      return setTimeout((function() {
        ...
        _ref8 = $(newNode).find('[data-org-comments]');
        _results = [];
        for (_k = 0, _len2 = _ref8.length; _k < _len2; _k++) {
          n = _ref8[_k];
          _results.push(setShadowHtml(n.firstElementChild, newCommentBox(n.getAttribute('data-org-comments'))));
        }
        return _results;
      }, redrawAllIssues()), 1);
    });
  ...
  toTestCaseButton = function(org) {
    if (isDef(org)) {
      ...
      return "<button class='testcase-button' onclick='Leisure.createTestCase(event)' contenteditable='false' data-org-commentcount='0'><img style='width: auto; height: 32px' src='images/toTestCaseButton.png'><span></span></button>";
    }
  };

  codeBlockForNode = function(node) {
    while (node && (typeof node.getAttribute === "function" ? node.getAttribute('data-org-type') : void 0) !== 'source') {
      node = node.parentNode;
    }
    if (node && !node.hasAttribute('org-test')) {
  ...
  createTestCase = function(evt) {
    var drawer, end, match, newExpectation, node, pre, rest, results, resultsText, start, text;
  ...
  htmlForResults = function(text) {
    return "</td><td><button class='results-indicator' onclick='Leisure.executeCode(event)' data-org-type='boundary'><img src='icons/monotone_arrow_right_next.png'></div></td><td><div class='coderesults' data-org-type='results'><span class='hidden'>#+RESULTS:\n</span><div class='resultscontent'><span></span><span class='hidden'>" + text + "</span></div></div>";
  };
  ...
  setTheme = function(str) {
    var el, node, _i, _len, _ref6;
    el = $('body');
    _ref6 = $('[data-org-headline="1"]');
    for (_i = 0, _len = _ref6.length; _i < _len; _i++) {
      node = _ref6[_i];
      if (node.shadowRoot) {
