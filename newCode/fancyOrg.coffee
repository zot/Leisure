...
markupSource = (org, name, intertext, delay)->
  ...
  if name
    ...
    codeBlock = " data-org-codeblock='#{escapeAttr name.info.trim()}'><div class='codename' contenteditable='true' data-newline><span class='hidden'>#{escapeHtml nameM[KW_BOILERPLATE]}</span><div><larger><b>#{escapeHtml name.info}</b></larger></div>#{escapeHtml intertext}</div>"
  ...
  wrapper += "</ul></td>"
  wrapper += "<td><div #{orgSrcAttrs org} contenteditable='true' data-newline>#{escapeHtml srcContent}</div><span class='hidden' data-org-type='boundary'>#{escapeHtml trail}</span>"
...
commentButton = (name)->
  "<button class='comment-button' onclick='Leisure.toggleComment(\"#{escapeAttr name}\", event)' contenteditable='false' data-org-commentcount='0'><i class='fa fa-comment'></i><div></div><span></span></button>"

toTestCaseButton = (org)->
  if isDef org then ''
  else "<button class='testcase-button' onclick='Leisure.createTestCase(event)' contenteditable='false' data-org-commentcount='0'><i class='fa fa-mail-forward'></i><i class='fa fa-bug'></i><div></div><span></span></button>"
