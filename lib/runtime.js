...
(function() {
  var Monad, Nil, ast2Json, basicRunMonad, booleanFor, cons, consFrom, defaultEnv, define, ensureLeisureClass, getDataType, getType, getValue, head, identity, left, makeMonad, monadQueue, nextMonad, none, parensContent, parensEnd, parensStart, readFile, replaceErr, right, root, runMonad, setType, setValue, some, strCoord, strFromList, strToList, tail, tokenPos, tokenString, values, wrapCont, wrapContAsyncQueue, wrapContAsyncTimeout, _, _false, _identity, _ref, _true,
    __slice = [].slice;
  ...
  defaultEnv = {
    ...
    asyncBind: function(state) {
      if (state) {
        return this.wrapCont = function(cont) {
          return wrapContAsyncTimeout(this, cont);
        };
      } else {
        return this.wrapCont = function(cont) {
          return wrapCont(this, cont);
        };
      }
    }
  };

  wrapCont = function(env, cont) {
    if (cont.wrap) {
      return cont;
    } else {
      cont.wrap = true;
  ...
  wrapContAsyncTimeout = function(env, cont) {
    if (cont.wrap) {
      return cont;
    } else {
      cont.wrap = true;
  ...
  wrapContAsyncQueue = function(env, cont) {
    if (cont.wrap) {
      return cont;
    } else {
      cont.wrap = true;
      return function() {
        ...
        args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        return monadQueue.push(cont, args);
      };
    }
  };
  ...
  basicRunMonad = function(monad, env, cont) {
    var err;

    env = env != null ? env : root.defaultEnv;
    cont = cont != null ? cont : identity;
    try {
      if (typeof monad === 'function' && (monad.cmd != null)) {
    ...
    return function(name) {
      return makeMonad(function(env, cont) {
        return readFile(name(), function(err, contents) {
          return wrapCont(env, cont)((err ? left(err.stack) : right(contents)));
        });
      });
    };
  });
