...
(function() {
  var Monad, Nil, ast2Json, booleanFor, codeMonad, cons, consFrom, define, ensureLeisureClass, getDataType, getType, getValue, head, identity, left, makeMonad, nextMonad, none, parensContent, parensEnd, parensStart, readFile, right, root, runCont, runMonad, setType, setValue, some, strCoord, strFromList, strToList, tail, tokenPos, tokenString, values, _, _false, _identity, _ref, _true,
    __slice = [].slice;
  ...
  runCont = function() {
    var args, cont, env, err;

    env = arguments[0], cont = arguments[1], args = 3 <= arguments.length ? __slice.call(arguments, 2) : [];
    try {
    ...
    return function(name) {
      return makeMonad(function(env, cont) {
        return readFile(name(), function(err, contents) {
          return runCont(env, cont, (err ? left(err.stack) : right(contents)));
        });
      });
    };
  });
  ...
  root.defaultEnv = {
    ...
    err: function(err) {
      return this.write("Error: " + err.stack);
    }
  };
