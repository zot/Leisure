    ...
    if (err) {
      ...
      return writeFile('/tmp/simpleParse.js', compileFile(code, "simpleParse.js"), function(err) {
        runTests('Basic', {
          ...
          basic13: function() {
            return assertEq(gen(lidAst), 'function(L_x){return L_x()}');
          },
          basic14: function() {
            return assertEq(gen(lapplyXY), 'function(L_x){return function(L_y){return L_x()(L_y)}}');
          },
          basic15: function() {
            return assertEq(gen(ltrueAst), 'function(L_a){return function(L_b){return L_a()}}');
          },
          basic16: function() {
        ...
        runTests('Leisure AST', {
          ...
          leisureAst25: function() {
            ...
            assertEq(String(lsr("scanLine " + (s('id = \\x . x')) + " " + delimiterPatStr + " id id")), "Cons[Token(\"define\", 5) Token(\"\\\"id\\\"\", 5) Token(\"0\", 5) Token(\"\\\"id = \\\\\\\\x . x\\\"\", 5) Cons[Token(\"\\\\@\", 5) Token(\"dataType\", 5) Token(\"id\", 5) Token(\".\", 5) Token(\"\\\\@\", 5) Token(\"type\", 5) Token(\"id\", 5) Token(\".\", 5) Token(\"\\\\\", 5) Token(\"x\", 6) Token(\".\", 8) Token(\"x\", 10)]]");
          ...
          leisureAst49: function() {
            ...
            assertEq(String(lsr("parseG " + (s('[a] [b]')) + " " + (s(LZ.delimiterPat.source)) + " (acons '[' ']' parenGroups)")), 'Cons[Parens(0, 3, Cons[Token("[", 0) Token("a", 1) Token("]", 2)]) Parens(4, 7, Cons[Token("[", 4) Token("b", 5) Token("]", 6)])]');
          ...
          leisureAst52: function() {
            ...
            return assertEq(String(lsr("postProcessMacro -1 -1 (cons (cons (token 'hello' 15) nil) (cons 'goodbye' nil))")), 'Cons[Cons[Token("hello", 15)] Token("goodbye", 21)]');
          },
        ...
        runTests('Leisure Utils', {
          leisureUtil1: function() {
            ...
            return assertEq(String(monad(lsr("quicksort lt (cons 2 (cons 3 (cons 1 nil)))"))), 'Cons[1 2 3]');
          }
        });
        return console.log('\nDone');
      });
