...
(function() {
  var Nil, action, ast2Json, compile, compileLines, createAstFile, createJsFile, defaultEnv, diag, evalInput, fs, gen, genJsFromAst, gennedAst, gennedJs, getMonadSyncMode, help, identity, interrupted, json2Ast, leisureCompleter, leisureFunctions, loadedParser, newCompileLines, newOptions, oldCompileLines, oldFunctionCount, outDir, path, primCompile, processArg, processedFiles, readFile, readline, recompiled, repl, replaceErr, rl, root, run, runMonad, setDataType, setType, stage, stages, tokenString, updateCompleter, usage, verbose, writeFile, _, _ref, _ref1, _ref2;
  ...
  _ref2 = require('./runtime'), identity = _ref2.identity, runMonad = _ref2.runMonad, defaultEnv = _ref2.defaultEnv, replaceErr = _ref2.replaceErr, getMonadSyncMode = _ref2.getMonadSyncMode;
  ...
  compile = function(file, cont) {
    ...
    return readFile(file, function(err, contents) {
      ...
      if (err) {
        ...
        asts = [];
        return compileLines(lines, names, asts, function(asts) {
          var outputFile;

          if (asts instanceof Error) {
            return cont(asts);
          } else {
            if (createAstFile) {
  ...
  oldCompileLines = function(lines, names, asts, cont) {
    if (lines.isNil()) {
      return cont(asts);
    } else {
      return runMonad(L_runLine()(function() {
        return names;
      })(function() {
        return lines.head();
      }), null, function(lineData) {
        var ast, result;
        ...
        if (result instanceof Error) {
          ...
          asts.push(ast);
          return compileLines(lines.tail(), names, asts, cont);
        }
      });
    }
  };

  newCompileLines = function(lines, names, asts, cont) {
    var ast, lineData, mode, result;

    while (!lines.isNil()) {
      console.log("RESETTING SYNC");
      mode = false;
      lineData = runMonad(L_runLine()(function() {
        return names;
      })(function() {
        return lines.head();
      }), null, function(data) {
