...
(function() {
  var Nil, SourceNode, action, ast2Json, asyncMonad, compile, createAstFile, createJsFile, defaultEnv, diag, evalInput, fs, gen, genJsFromAst, genNode, gennedAst, gennedJs, getMonadSyncMode, getParseErr, getType, help, identity, interactive, interrupted, intersperse, isMonad, json2Ast, lazy, leisureCompleter, leisureFunctions, loadedParser, lz, newOptions, newRunMonad, oldFunctionCount, outDir, path, primCompile, processArg, processedFiles, prog, prompt, promptText, readFile, readline, recompiled, repl, replEnv, replaceErr, requireList, resolve, rl, root, run, runFile, runMonad, rz, setDataType, setType, setWarnAsync, sourceNode, stage, stages, tokenString, updateCompleter, usage, verbose, writeFile, _, _ref, _ref1, _ref2, _ref3, _ref4;
  ...
  _ref2 = require('./gen'), gen = _ref2.gen, genNode = _ref2.genNode, sourceNode = _ref2.sourceNode;
      ...
      try {
        ...
        return runMonad(result, replEnv, function(ast) {
          ...
          try {
            ...
            err = _error;
            return cont("ERROR: " + ((_ref5 = err.stack) != null ? _ref5 : err));
          }
        });
      } catch (_error) {
        err = _error;
      ...
      if (createJsFile) {
        ...
        result = (new SourceNode(1, 0, bareLsr, [
          "L_runMonads([\n  ", intersperse(_(asts).map(function(item) {
            return sourceNode(item, "function(){return ", gen(item), "}");
          }), ',\n '), "]);\n"
        ])).toStringWithSourceMap({
          file: bareJs
        });
        console.log("FILE: " + outputFile + ", MAP: " + outputMap);
        return writeFile(outputFile, result.code, function(err) {
          if (!err) {
