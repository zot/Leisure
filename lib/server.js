...
(function() {
  var URI, attemptCollab, createNamespace, createServer, diff, dir, fs, getFile, handler, hashes, io, newTrunk, ns, root, sha, storeVersion, trunks, util, yaml, _;
  ...
  handler = function(req, res) {
    ...
    } else if (m = uri.path.match(/^((newCode\/|core\/).*)$/)) {
      return getFile(uri, req, res, m);
  ...
  createNamespace = function(file) {
    ...
    return ns.on('connection', function(socket) {
      ...
      return socket.on('storeData', function(_arg) {
        var key, value, yamlValue;
        key = _arg.key, value = _arg.value, yamlValue = _arg.yamlValue;
        return ns.broadcase('receiveData', {
          key: key,
          value: value,
          yaml: yamlValue
        });
  ...
  storeVersion = function(socket, hash, parent, patch, keepPrivate, mergeHash) {
    var d, sock, sockName, _ref, _results;
    ...
    socket.hash = hash;
    if (!keepPrivate) {
      _ref = socket.namespace.sockets;
      _results = [];
      for (sockName in _ref) {
        sock = _ref[sockName];
  ...
  createServer = function(opts) {
    ...
    app.listen((_ref = opts.port) != null ? _ref : 80);
