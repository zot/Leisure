...
(function() {
  var L_parens, L_parseErr, L_token, Nil, addDelimiter, anno, apply, arity, ast2Json, chainApply, checkSetDataType, checkSetDataTypeFurther, cleanTokens, compileFile, compileLine, cons, consFrom, createAnno, createApply, createAst, createLambda, createLet, createLitOrRef, createSublets, defPat, define, delimiterList, delimiterListPrefix, delimiterPat, delimiters, ensureLeisureClass, gen, genLine, getLetLambda, getLetNames, getNameAndDef, head, identity, isParens, isToken, isTokenStart, isTokenString, jsonForFile, lambda, linesForFile, lit, llet, loc, makeDelimterPat, makeMoreTokens, makeTokens, parens, parensContent, parensEnd, parensFromToks, parensStart, parse, parseErr, parseErrMsg, parseGroup, parseLine, parseToAst, parseTok, parseToks, position, ref, root, scanLine, scrub, setDataType, setDataTypeAnno, setDefAnno, setType, setTypeAnno, splitTokens, strip, tail, tokListStr, token, tokenPos, tokenString, tokens, transformDef, withCons, withParens, withToken, _, _ref,
  ...
  makeMoreTokens = function makeMoreTokens(strings, start) {
    if (head(strings)[0] === ' ') {
  ...
  isTokenStart = function isTokenStart(t, str) {
    return isToken(t) && tokenString(t).indexOf(str) === 0;
  };
  ...
  parseToks = function parseToks(toks, indent, cont) {
    if (toks === Nil) {
      return cont(Nil);
    } else {
      return parseTok(toks, indent, function(h, t) {
        return parseToks(t, indent, function(res) {
          if (!cont) throw new Error("No cont function!");
  ...
  parseTok = function parseTok(toks, indent, cont) {
    return withCons(toks, (function() {
      return Nil;
    }), function(h, t) {
      if (isTokenString(h, '(')) {
        return parseGroup(h, t, indent, Nil, cont);
      } else if (isTokenStart(h, ' ')) {
        return parseTok(t, indent, cont);
      } else {
        return cont(h, t);
      }
    });
  ...
  parseGroup = function parseGroup(left, toks, indent, gr, cont) {
    return withCons(toks, (function() {
      ...
      if (isTokenString(h, ')')) {
        ...
        return parseTok(toks, indent, function(restH, restT) {
          return parseGroup(left, restT, indent, cons(restH, gr), cont);
        });
      }
    });
  };
  ...
  parse = function parse(str) {
    return parseToks(tokens(str), 0, identity);
  };
  ...
  scanLine = function scanLine(str, isDef, isExpr) {
    var func, name, toks;
    toks = tokens(str);
    if (str.match(defPat)) {
      name = head(toks);
      ...
      return parseToks(setDefAnno(checkSetDataType(func, tail(toks), name), name, arity(tail(toks), 0), str), 0, function(list) {
        return isDef(list);
      });
    } else {
      return parseToks(toks, 0, function(list) {
        return isExpr(list);
      });
    }
  };
  ...
  parseLine = function parseLine(str, isDef, isExpr) {
    var astCallback;
    astCallback = function astCallback(cb) {
  ...
  linesForFile = function linesForFile(text) {
