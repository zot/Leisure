...
(function() {
  var L_parens, L_parseErr, L_token, Nil, addDelimiter, anno, apply, arity, chainApply, checkSetDataType, checkSetDataTypeFurther, cleanTokens, compile, cons, consFrom, createAnno, createApply, createAst, createLambda, createLet, createLitOrRef, createSublets, defPat, define, delimiterList, delimiterPat, delimiters, ensureLeisureClass, gen, getLetLambda, getLetNames, getNameAndDef, head, identity, isParens, isToken, isTokenString, lambda, lit, llet, loc, makeDelimterPat, makeTokens, parens, parensContent, parensEnd, parensFromToks, parensStart, parse, parseErr, parseErrMsg, parseGroup, parseToAst, parseTok, parseToks, position, ref, root, setDataType, setDataTypeAnno, setType, setTypeAnno, splitTokens, strip, tail, tokListStr, token, tokenPos, tokenString, tokens, transformDef, withCons, withParens, withToken, _, _ref;

  _ref = root = module.exports = require('./ast'), cons = _ref.cons, Nil = _ref.Nil, consFrom = _ref.consFrom, head = _ref.head, tail = _ref.tail, define = _ref.define, setDataType = _ref.setDataType, setType = _ref.setType, ensureLeisureClass = _ref.ensureLeisureClass, ref = _ref.ref, lit = _ref.lit, apply = _ref.apply, lambda = _ref.lambda, llet = _ref.llet, anno = _ref.anno;
  ...
  compile = function compile(str, isDef, isExpr) {
    var func, name, toks;
    toks = tokens(str);
    if (str.match(defPat)) {
      name = head(toks);
      ...
      return parseToks(checkSetDataType(func, toks, name), function(list) {
        return createAst(list, Nil, function(ast) {
          return isDef(eval("define('" + (tokenString(name)) + "', (function(){return " + (gen(ast)) + "}), " + (arity(tail(toks), 0)) + ", " + (tokListStr(func)) + ")"));
        });
      });
    } else {
      return parseToks(toks, function(list) {
        return createAst(list, Nil, function(ast) {
          return isExpr(eval("(function(){return " + (gen(ast)) + ";})"));
        });
      });
    }
  };
  ...
  setTypeAnno = function setTypeAnno(start, toks, name) {
    var pos;
    pos = position(start);
    return cons(token('\\@', pos), cons(token('type', pos), cons(token(name, pos), cons(token('.', pos), toks))));
  };
