...
(function() {
  var L_parens, L_parseErr, L_token, Nil, addDelimiter, anno, apply, arity, ast2Json, chainApply, charCodes, checkSetDataType, cleanTokens, compileFile, compileLine, cons, consFrom, createAnno, createApply, createAst, createLambda, createLet, createLitOrRef, createSublets, defPat, define, delimiterList, delimiterListPrefix, delimiterPat, delimiters, ensureLeisureClass, gen, genLine, getLetLambda, getLetNames, getNameAndDef, head, identity, isParens, isToken, isTokenStart, isTokenString, jsonForFile, lambda, lexEnd, linesForFile, lit, llet, loc, makeDelimterPat, makeMoreTokens, makeTokens, namesForLines, parens, parensContent, parensEnd, parensFromToks, parensStart, parse, parseErr, parseErrMsg, parseGroup, parseIndent, parseLine, parseString, parseToAst, parseTok, parseToks, position, ref, root, scanLine, setDataType, setDataTypeAnno, setDefAnno, setType, setTypeAnno, splitTokens, strip, stripParens, tail, tokListStr, token, tokenPos, tokenString, tokens, transformDef, withCons, withParens, withToken, _, _ref,
  ...
  makeMoreTokens = function(strings, start) {
    ...
    first = head(strings);
    if (_ref1 = first[0], __indexOf.call(' #', _ref1) >= 0) {
  ...
  compileFile = function(text) {
    ...
    return _.map(lines, function(line) {
      return genLine(line.trim(), names, id, id);
    }).join('');
  };
