...
(function() {
  ...
  markupSource = function(org, name, intertext, delay) {
    var channels, codeBlock, contHtml, expected, finalIntertext, fluff, lead, nameM, node, resText, result, srcContent, startHtml, testAttr, testCase, testCaseButton, testValue, top, trail, wrapper, _ref7;
  ...
  fancyCheckSourceMod = function(focus, div, currentMatch, el) {
    ...
    if (code = getCodeContainer(focus)) {
      ...
      prefix = "#+NAME: ";
      restorePosition(el.parentNode.parentNode, prefix.length, function() {
        var txt;
        txt = el.parentNode.textContent;
        return el.parentNode.textContent = "" + prefix + txt + (!txt.match(/\n/) ? '\n' : '');
      });
