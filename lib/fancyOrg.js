...
(function() {
  ...
  markupHeadline = function(org) {
    return "<div " + (orgAttrs(org)) + " data-org-headline='" + org.level + "'><span data-org-type='text'>" + org.text + "</span>" + (markupGuts(org, checkStart(start, org.text))) + "</div>";
  };

  markupSource = function(org, name, intertext) {
    var codeBlock, html, nameM, pos, res, srcContent, srcLead, srcM, srcTrail;
  ...
  commentButton = function(name) {
    return "<button class='comment-button' onclick='Leisure.toggleComment(\"" + name + "\")' contenteditable='false' data-org-commentcount='0'><img src='icons/monotone_talk_chat_speech.png'><span></span></button>";
  };

  commentBlock = function(name) {
    return "<div class='comments' data-org-comments='" + name + "'><div></div></div>";
  };
  ...
  toggleComment = function(name) {
    ...
    if (block.hasClass('showcomments')) {
      ...
      return $("[data-org-codeblock='" + name + "'] button.comment-button").removeClass('new-comments');
    }
  };
