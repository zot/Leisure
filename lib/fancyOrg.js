...
(function() {
  ...
  markupSource = function(org, name, intertext) {
    var codeBlock, contHtml, expected, lead, nameM, node, resText, result, srcContent, startHtml, testCase, trail, wrapper, _ref7;
    ...
    wrapper += "</td></tr></table>";
    testCase = ((_ref7 = resultsType(org)) === 'test' || _ref7 === 'autotest') && expected && (resText != null);
  ...
  commentButton = function(name) {
    return "<button class='comment-button' onclick='Leisure.toggleComment(\"" + (escapeAttr(name)) + "\", event)' contenteditable='false' data-org-commentcount='0'><img src='icons/monotone_talk_chat_speech.png'><span></span></button>";
  };
    ...
    if (block.hasClass('showcomments')) {
      ...
      return replacePresenter({
        ...
        isRelated: function(target) {
          return button === $(target).closest('button')[0];
        }
      });
    }
  };
  ...
  createResults = function(srcNode) {
    var created, _ref7;
    while (srcNode && !((_ref7 = srcNode.classList) != null ? _ref7.contains('codeblock') : void 0)) {
  ...
  runAutotests = function(doc) {
    ...
    _ref7 = $(doc).find("[data-org-results='autotest']");
    _results = [];
    for (_i = 0, _len = _ref7.length; _i < _len; _i++) {
      n = _ref7[_i];
  ...
  redrawIssue = function(issue) {
    ...
    if ((name = $("[data-org-comments='" + issueName + "']")).length) {
      ...
      return setShadowHtml(name[0].firstChild, "" + (commentHtml(issue, 'main')) + (((function() {
        ...
        return _results;
      })()).join('')) + (newCommentBox(issueName, $(name[0].parentNode).find('.codeblock').attr('id'))));
    }
  };
