...
(function() {
  var BS, DEL, ENTER, HL_TAGS, Headline, KW_BOILERPLATE, KW_INFO, KW_NAME, Keyword, Meat, Results, Source, TAB, addKeyPress, atTextEnd, atTextStart, bindContent, borderRE, boundarySpan, bsWillDestroyParent, checkCollapsed, checkEnterReparse, checkExtraNewline, checkSourceMod, checkStart, commentHtml, content, contentSpan, createResults, crossesHidden, curPos, currentLine, currentTextPosition, defaultBindings, defaultEnv, defaultMarkup, displaySource, executeSource, executeText, fancyOrg, findKeyBinding, followingSpan, getOrgParent, getOrgType, getResultsForSource, handleMutation, headlineRE, invalidateOrgText, isCollapsed, keyFuncs, keywordRE, lastOrgOffset, lazy, lz, makeBoundary, markupGuts, markupNode, markupOrg, markupOrgWithNode, matchLine, matchLineAt, modifiers, modifyingKey, nextOrgId, optionalBoundary, orgAttrs, orgEnv, orgNotebook, parseOrgMode, parseTags, processResults, redrawAllIssues, redrawIssue, reprocessResults, resolve, root, rz, setCurKeyBinding, shouldCancelBS, shouldCancelDEL, srcStartRE, swapMarkup, toggleComment, _, _ref, _ref1, _ref2, _ref3;
    ...
    if (org.offset <= lastOrgOffset) {
      ...
      if (org.name.match(/name/i)) {
        ...
        if (src instanceof Source) {
          ...
          return html + ("</div><button class='comment-button' onclick='Leisure.toggleComment(\"" + (name.info.trim()) + "\")'><img src='icons/monotone_talk_chat_speech.png'></button></div><div class='comments' data-org-comments='" + (name.info.trim()) + "'><div></div></div>");
        } else {
