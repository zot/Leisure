;;; leisure-connection-mode.el --- server for connections to Leisure
  ...
  (let* ((bufferName (leisure/conInfo-bufferName conInfo))
    ...
    (cond ((and (equal bufferName requestedName)
          ...
          (t (if (not (or (leisure/str bufferName) (leisure/str requestedName)))
             ...
             (let ((oldBuf (get-buffer bufferName)))
               (get-buffer-create bufferName)
               (with-current-buffer bufferName
                 (if (not oldBuf) (org-mode))
...
(defun leisure-reload ()
  "Reload the connected Leisure page"
  (interactive)
  (if leisure/bufferConnection
      (let ((ws (leisure/conInfo-websocket leisure/bufferConnection)))
        (if ws
            (progn (websocket-send-text ws "reload")
                   (leisure/diag leisure/bufferConnection "sent reload"))))))
...
(defun leisure-readme ()
  ...
  (interactive)
  (switch-to-buffer (make-temp-file "Leisure-readme"))
  (insert-file-contents (format "%sREADME.org" (file-name-directory (find-library-name "leisure-connection-mode")))))

(provide 'leisure-connection-mode)
