...
term for interactive, collboratively editable documents that are
applications, source code, and data, all rolled up into one.  Sort of
a zero-install version of reproducible research.

This idea has been around for quite a while (Hypercard and Mathematica
did it, albeit without collaboration).  We thought it was well past
time for an open-source, web-based *Illuminated Programming*
