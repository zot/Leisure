...
(function() {
  ...
  */
  ...
  ({newCall} = requirejs('base'));

  ({cons, Nil, consFrom, head, tail, define, setDataType, setType, ensureLeisureClass, ref, lit, apply, lambda, llet, anno, ast2Json, resolve, lazy, dummyPosition} = root = module.exports = requirejs('./ast'));
  ...
  ({runMonad2, defaultEnv, identity} = requirejs('./runtime'));
  ...
  makeMoreTokens = function(strings, start) {
    var first, ref1;
    first = head(strings);
    if (ref1 = first[0], indexOf.call(' #', ref1) >= 0) {
  ...
  createLitOrRef = function(tok, names, cont) {
    ...
    if (names.find(function(el) {
      ...
      try {
        if (ref1 = tok[0], indexOf.call("\"'", ref1) >= 0) {
  ...
  chainApply = function(func, list, names, cont) {
    return withCons(list, (function() {
      ...
      var ref1;
      if (isToken(argItem) && ((ref1 = tokenString(argItem)) === '\\' || ref1 === '\\\\' || ref1 === '\\@')) {
