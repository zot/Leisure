...
- Each document is like a tiny, shared virtual machine, with code, data, presentation, and it also *runs*
...
- Collaborative and allows local data
...
data: encodeURIComponent [29,89].join '\n'
...
* Test Cases
#+NAME: chet
#+BEGIN_SRC coffee :results yaml dynamic   view(testCase)
firstName: 'Bob'
lastName: 'Jones'
#+END_SRC
:expected:
...
{{#if (isExpected)}}<span class='test pass'>PASS</span>{{else}}<span class='test fail'>FAIL</span>{{/if}}
#+END_SRC
