...
roll = do
  ...
  result <- do
    ...
    size = (sizeCount + 2) * 2
    dice <- foreach repeat (afetch size selections)
    valueRand <- rand
    angleRand <- rand
    #print concat['roll die #' dice ': ' valueRand]
    [{'size':size 'value':(ceil valueRand * size) 'angle':(floor angleRand * 24)}]
  print concat['roll: ' result]
  debug
  changeData
    ...
    newRoll = {'type':'roll' 'number':count 'player':(afetch 'player' player) 'roll':result}
