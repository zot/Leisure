...
(function() {
  define(['./lib/lodash.min', 'cs!./export.litcoffee', 'cs!./ui.litcoffee', 'cs!./editor.litcoffee'], function(_, Exports, UI, Editor) {
    var addBlockOffset, blockRangeFor, c, close, configureEmacs, connect, connected, e, error, escapeString, escaped, findEditor, mergeExports, message, messages, msgPat, offsetFor, open, replace, replaceMsgPat, replaceWhile, replacing, sendReplace, slashed, specials, unescapeString, unescaped;
    ...
    replace = function(data, msg) {
      ...
      return replaceWhile(function() {
        if (end === -1) {
          return editor.load(text);
        } else {
          return editor.replace(null, blockRangeFor(data, Number(start), Number(end)), text);
        }
      });
    };
    ...
    sendReplace = function(con, start, end, text) {
      ...
      return console.log("sending r " + start + " " + end + " " + (JSON.stringify(text)));
    };
