...
(function() {
  define(['./lib/lodash.min', './export', './ui', './editor', './editorSupport', './diag', './eval', './advice', './lib/fingertree'], function(_, Exports, UI, Editor, EditorSupport, Diag, Eval, Advice, Fingertree) {
    var AtomSupport, LINE_END, NON_LEISURE_LINE_END, Point, basicDataFilter, configureAtom, getDocumentParams, leisureText;
    if (!window.atomView) {
      return {
        ...
        if (document.location.search.length > 1 && !connected) {
          ...
          if (con) {
            ...
            if (u.protocol === 'atom:' && (m = u.pathname.match(/^\/\/(.*)$/))) {
              ...
              return opts.load('atom', leisureText(text));
            }
          }
        }
      });
