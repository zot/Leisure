...
(function() {
  ...
  define(['./base', './org', './docOrg', './ast', './eval', './editor', 'lodash', 'jquery', './ui', 'handlebars', './lib/prism', './editorSupport', 'bluebird', './advice', './lib/prism-leisure', 'lib/js-yaml'], function(Base, Org, DocOrg, Ast, Eval, Editor, _, $, UI, Handlebars, Prism, EditorSupport, Bluebird, Advice, PrismLeisure, Yaml) {
    ...
    ({parseOrgMode, parseMeat, Fragment, Headline, SimpleMarkup, Link, Keyword, ListItem, Drawer, Meat, Example, HTML, Nil, headlineRE, HL_LEVEL, HL_TODO, HL_PRIORITY, HL_TEXT, HL_TAGS, keywordRE, KW_BOILERPLATE, KW_INFO, KEYWORD_} = Org);
    ({orgDoc, getCodeItems, blockSource, parseYaml} = DocOrg);
    ({Nil} = Ast);
    ({languageEnvMaker, Html, escapeHtml, html, blockVars, isYamlResult, unescapeString} = Eval);
    ({LeisureEditCore, last, DataStore, DataStoreEditingOptions, blockText, posFor, escapeHtml, copy, findEditor, copyBlock, preserveSelection, getEventChar} = Editor);
    ({addView, removeView, renderView, hasView, viewKey, addController, removeController, withContext, mergeContext, initializePendingViews, escapeAttr, nextImageSrc, prevImageSrc, pushPendingInitialzation, nextViewId, touchedBlocks} = UI);
    ({OrgEditing, blockOrg, blockCodeItems, blockIsHidden, blockEnvMaker, controllerEval, updateSelection} = EditorSupport);
    ({Promise} = Bluebird);
    ({dump} = Yaml);
    singleControllers = {};
    numPat = /-?[0-9][0-9.]*|-?\.[0-9.]+/;
    ...
    plainMode = {
      ...
      render: function(opts, block, prefix, replace) {
        var attrs, error, name, pos, ref, ref1, ref2, ref3, result, results, source, text;
        ...
        if (!results && !error) {
          ...
          if (!error) {
            text += this.renderMainText(block.text.substring(source.offset, (ref = results != null ? results.offset : void 0) != null ? ref : block.text.length));
          } else {
            pos = source.offset + source.contentPos + Number(error.info.match(/([^,]*),/)[1]) - 1;
            text = escapeHtml(block.text.substring((ref1 = name != null ? name.offset : void 0) != null ? ref1 : 0, pos)) + "<span class='errorMark' contenteditable='false' data-noncontent>✖</span>" + escapeHtml(block.text.substring(pos, (ref2 = results != null ? results.offset : void 0) != null ? ref2 : block.text.length));
          }
          if (results != null) {
    ...
    Handlebars.registerHelper('yaml', function(item, opts) {
      console.log(`YAML: ${item}`);
      //escapeHtml dump item
      return `<pre class='name-label' data-org-src style='height: initial'>${prismHighlight('yaml', dump(item))}</pre>`;
    });
    ...
    Handlebars.registerHelper('renderHtml', function(html, options) {
      var controllerName, data, id, ids, opts, ref, ref1, ref10, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, tblocks, text, vars;
      opts = (ref = UI.context) != null ? ref.opts : void 0;
      [vars, ids] = blockVars(opts != null ? opts.data : void 0, (ref1 = this.block) != null ? (ref2 = ref1.codeAttributes) != null ? ref2.var : void 0 : void 0);
      data = (ref3 = UI.context) != null ? (ref4 = ref3.opts) != null ? ref4.data : void 0 : void 0;
      controllerName = (ref5 = this.block) != null ? (ref6 = ref5.codeAttributes) != null ? ref6.controller : void 0 : void 0;
      ...
      if (html) {
        ...
        if (controllerName || (ids.length > 0 && (id = (ref9 = (ref10 = UI.context) != null ? ref10.simpleViewId : void 0) != null ? ref9 : this.id)) || !_.isEmpty(tblocks)) {
          pushPendingInitialzation(() => {
            var blk, block, blocks, controller, env, j, len, len1, n, node, p, ref11, ref12, ref13, s, viewNode;
            viewNode = $(`#${id}`);
            if (ids.length && (node = opts.nodeForId(this.block._id)) && (node[0] === viewNode[0] || node[0].compareDocumentPosition(viewNode[0]) & Element.DOCUMENT_POSITION_CONTAINS)) {
            ...
            if (!_.isEmpty(tblocks)) {
              s = new Set();
              ref13 = ((ref12 = viewNode.attr('data-observe')) != null ? ref12 : '').split(/\s+/);
              for (n = 0, len1 = ref13.length; n < len1; n++) {
                blk = ref13[n];
                s.add(blk);
              }
              for (blk in tblocks) {
    ...
    Handlebars.registerHelper('renderPlain', function(data) {
      var block, edata, end, next, plainText, ref, text;
      text = '';
    ...
    Handlebars.registerHelper('renderSource', function() {
      var error, ignore, msg, pos, source, src;
      ({error, source} = this.codeItems);
    ...
    fancyMode = {
      ...
      renderBlocks: function(opt, html) {
        var header;
        header = hasView('header') ? opt.withNewContext(() => {
          return this.renderView('header', null, null, {})[0];
        }) : "<div id='dummy_headline'></div>";
        return `${header}${html}`;
      },
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          ...
          if (changes.sets[block._id] && (old = opts.getBlock(block._id)) && isHiddenSlide(block) !== isHiddenSlide(old)) {
            if (isHiddenSlide(block)) {
              ...
              siblingNode = null;
              [newNode] = this.render(opts, block, opts.prefix);
              sibling = block;
      ...
      render: function(opts, block, prefix, replace) {
        ...
        if (opts.shouldHide(block) || block.local) {
          ...
          return opts.withNewContext(() => {
            var result;
            UI.context.currentView = opts.nodeForId(block._id);
            UI.context.block = block;
            addPendingTooltip(opts, block._id);
            result = block.type === 'headline' ? this.renderHeadline(opts, block, prefix, replace) : !block.prev ? this.renderFirstBlocks(opts, block, prefix, replace) : this.renderNontop(opts, block, prefix, replace);
            if (replace) {
              validateBlock(opts, block);
            }
            return result;
      ...
      renderHeadline: function(opts, block, prefix, replace) {
        ...
        if (hasView(viewName)) {
          ...
          id = block.next;
          while (id && id !== next) {
            [nextText, id] = this.render(opts, opts.data.getBlock(id), prefix);
            text += nextText;
          }
          text += "</span>";
      ...
      renderFirstBlocks: function(opts, block, prefix, replace) {
        var cur, next, plain, ref, targets, text, txt;
        if (hasView('leisure-top-chunk')) {
          ...
          while (!(cur.type === 'headline' && cur.level === 1)) {
            [txt, next] = plain ? plainMode.render(opts, cur, prefix) : this.renderNontop(opts, cur, prefix);
            text += txt;
            if (!next) {
              break;
            }
      ...
      renderCode: function(opts, block, prefix, replace) {
        var error, hideResults, items, key, lang, m, name, nameBoiler, org, ref, ref1, ref2, results, source, sourceData, targets;
        key = "leisure-code";
        org = blockOrg(opts.data, block);
        hideResults = !showsResults(block);
        ({name, source, error, results} = items = getCodeItems((ref = (ref1 = org.children) != null ? ref1[0] : void 0) != null ? ref : org));
        ...
        if (hasView(key, lang)) {
          ...
          sourceData = {
            ...
            beforeResults: block.text.substring(0, (ref2 = results != null ? results.offset : void 0) != null ? ref2 : source.end())
          };
      ...
      renderCodeOrg: function(opts, context) {
        var block, error, name, pos, results, source, text;
        block = context.block;
        ({name, source, error, results} = context.codeItems);
        text = '';
        pos = 0;
        [pos, text] = this.renderCodeSegment(opts, 'name', pos, text, context);
        [pos, text] = this.renderCodeSegment(opts, 'source', pos, text, context);
        [pos, text] = this.renderCodeSegment(opts, 'error', pos, text, context);
        [pos, text] = this.renderCodeSegment(opts, 'results', pos, text, context);
      ...
      renderOrg: function(opts, org, start, noName) {
        var boiler, child, i, name, nameText, named, text;
      ...
      renderLink: function(opts, org) {
        ...
        if (leisureMatch = org.isLeisure()) {
          ...
          if (error) {
            ...
            return `<span class='error link' title='${escapeAttr(error)}'${attrs}><b data-noncontent contenteditable='false'>✖</b>${fancyHtml(org.allText())}</span></span>`;
          } else {
    ...
    doSlideValue = function({slaveId}, opts, start, offset, numberCount, numberOffset, delta) {
      var block, cur, dist, j, len, m, newText, node, numCur, number, numbers, sliderNode, targetDistance;
    ...
    getSliderPosition = function() {
      var block, cs, cursor, j, len, node, nth, number, numbers, offset, opts, sliderNode, start;
      cs = currentSlider;
      start = cs.data.getMarkLocation('__slider__');
    ...
    // Exports
    Object.assign(Leisure, {plainMode, fancyMode, toggleSlideMode, blockVars, createValueSliders, slideValue, mayHideValueSlider, setSliding, cleanOrg, showsCode, showsResults});
    return {plainMode, fancyMode, plainEditDiv, fancyEditDiv, doSlideValue};
  });
