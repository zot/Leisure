...
(function() {
  var MasterHandler, MessageHandler, SlaveHandler, _, badIdError, badMsgTypeError, badVersionError, crypto, diag, disapprovedError, guid, http, isTextMsg, masters, ref, requirejs, runReplacements, s4, slaves, socketPrefix, sockjs, sockjsUtil, startServer, validateBatch,
    slice = [].slice,
  ...
  MasterHandler = (function(superClass) {
    ...
    MasterHandler.prototype.updateDoc = function(changes) {
      ...
      for (i = ref1.length - 1; i >= 0; i += -1) {
        repl = ref1[i];
        this.doc = this.doc.substring(0, repl.start) + repl.text + this.doc.substring(repl.end);
      }
      this.unreplacements = [];
      return runReplacements(changes || this.versionOps, (function(_this) {
        return function(start, end, text) {
          ...
          return _this.doc = _this.doc.substring(0, start) + text + _this.doc.substring(end);
        };
      })(this));
    };
