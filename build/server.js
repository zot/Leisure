...
(function() {
  var MasterHandler, MessageHandler, SlaveHandler, _, badIdError, badMsgTypeError, crypto, guid, http, isTextMsg, masters, ref, s4, slaves, socketPrefix, sockjs, sockjsUtil, startServer, validateBatch,
  ...
  MessageHandler = (function() {
    ...
    MessageHandler.prototype.handler = {
      ...
      conditionalReplace: function(msg) {
        if (msg.targetChange === this.master.changeCount) {
          return this.master.relay(msg);
        } else {
          return this.send({
            type: 'rejectChange',
            change: msg.targetChange
          });
        }
      }
    };

    MessageHandler.prototype.sendEcho = function(change, target) {
      var msg;
      msg = {
        type: 'echo',
        change: change
  ...
  MasterHandler = (function(superClass) {
    ...
    MasterHandler.prototype.relay = function(msg) {
      var end, i, id, len, offset, ref1, ref2, repl, replId, results, slave, start, target, text;
      replId = msg.connectionId;
      if (target = msg.targetChange) {
        delete msg.targetChange;
      }
      if (msg.type === 'replace') {
      ...
      msg.change = ++this.changeCount;
      msg.msgCount = this.msgCount++;
      if (this.shouldEcho(msg)) {
        this.sendEcho(msg.change, target);
      } else {
        this.send(msg);
        if ((slave = this.slaves[replId]).shouldEcho(msg)) {
