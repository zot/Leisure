...
(function() {
  ...
  define(['./base', './ast', './runtime', './gen', './eval', './org', './transaction'], function(Base, Ast, Runtime, Gen, Eval, Org, Transaction) {
    ...
    dispatchCollaborative = function(opts, name, args) {
      return new Monad2("collaborative-" + name, function(env, cont) {
        return env.opts.doCollaboratively(name, _.map(args, function(el) {
          return L_toJson(el);
    ...
    defaultEnvWithOpts = function(opts) {
      var env;
      env = Object.create(defaultEnv);
      env.opts = opts;
      return env;
    };
    define('registerCollaborative', function(name, func) {
      return new Monad2('registerCollaborative', function(env, cont) {
        var dispatchFunc, dispatchSrc, i, opts;
        opts = env.opts;
        ...
        opts.registerCollaborativeCode(name, function() {
          ...
          return runMonad2(func.apply(null, cvtArgs), defaultEnvWithOpts(opts), function() {});
        });
        return cont(_unit);
      });
    });
    ...
    define('setData', function(name, value) {
      ...
      if (r = doPartial(arguments)) {
        ...
        return new Monad2('setData', function(env, cont) {
          return cont(jsonConvert(env.opts.setData(rz(name), L_toJson(rz(value)))));
        });
      }
    });
    define('setLocalData', function(name, value) {
      ...
      if (r = doPartial(arguments)) {
        ...
        return new Monad2('setLocalData', function(env, cont) {
          return cont(jsonConvert(env.opts.setLocalData(rz(name), L_toJson(rz(value)))));
        });
      }
    });
