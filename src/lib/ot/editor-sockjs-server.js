// Bill Burdick changed this to fit SockJS, 10/11/2015
...
function EditorSockJsServer (document, operations, docId, mayWrite) {
...
util.inherits(EditorSockJsServer, Server);
extend(EditorSockJsServer.prototype, EventEmitter.prototype);
...
EditorSockJsServer.prototype.addClient = function (socket) {
  var self = this;
  socket
    .join(this.docId)
    .emit('doc', {
...
EditorSockJsServer.prototype.onOperation = function (socket, revision, operation, selection) {
  var wrapped;
  try {
...
EditorSockJsServer.prototype.updateSelection = function (socket, selection) {
  var clientId = socket.id;
  if (selection) {
...
EditorSockJsServer.prototype.setName = function (socket, name) {
  var clientId = socket.id;
  this.getClient(clientId).name = name;
  socket.broadcast['in'](this.docId).emit('set_name', clientId, name);
};

EditorSockJsServer.prototype.getClient = function (clientId) {
  return this.users[clientId] || (this.users[clientId] = {});
};

EditorSockJsServer.prototype.onDisconnect = function (socket) {
  var clientId = socket.id;
  delete this.users[clientId];
  socket.broadcast['in'](this.docId).emit('client_left', clientId);
};

module.exports = EditorSockJsServer;
